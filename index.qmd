...
1. **What does the data show?**
   ...
   legend_elements = [
       ...
       mlines.Line2D([], [], color="#1f77b4", linewidth=5, alpha=0.25, label='Range (Min–Max)'),
   ]
...
Shop 3 benefits the least from boss presence -- likely due to the boss's brother working there, acting as a substitute effect for boss presence. 
...
2. **What should Patrick do?**
   ...
   ```{python}
   import numpy as np
   # Create columns with clearer names for the simulation below
   carsDF['shop'] = carsDF['shopID']
   carsDF['bossPresent'] = carsDF['boss']

   # Simulate 100 days: Boss ONLY at Shop 4, using bootstrap from carsDF
   n_days = 100
   simulated_prod = []
   ...
   # Prepare samples: for each shop, get the relevant subset of carsFixed
   samples = {}
   for shop in shop_list:
       if shop == 4:
           # Boss present at Shop 4
           mask = (carsDF['shop'] == 4) & (carsDF['bossPresent'] == 1)
       else:
           # Boss absent at other shops
           mask = (carsDF['shop'] == shop) & (carsDF['bossPresent'] == 0)
       shop_vals = carsDF.loc[mask, 'carsFixed'].values
       samples[shop] = shop_vals

   for _ in range(n_days):
       day_prod = 0
       for shop in shop_list:
           shop_sample = samples[shop]
           if len(shop_sample) == 0:
               # If there's no data, skip (shouldn't happen in practice)
               continue
           # Bootstrap sample: randomly pick one with replacement
           val = rng.choice(shop_sample)
           day_prod += val
       simulated_prod.append(day_prod)

   # Plot line chart
   import matplotlib.pyplot as plt
   plt.figure(figsize=(8,3.5))
   plt.plot(range(1, n_days+1), simulated_prod, color="#1f77b4", linewidth=2)
   plt.title("Simulated Total Cars Fixed per Day\nIf Boss Always Visits Shop 4 (100 Days)")
   plt.xlabel("Simulated Day")
   plt.ylabel("Total Cars Fixed (All Shops)")
